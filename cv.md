---
name: "William L. Gross, MD PhD"
affiliation: "Medical College of Wisconsin"
address1: "8701 W Watertown Plank Rd"
address2: "Milwaukee, WI 53226"
email: "bgross@mcw.edu"
phone: "414-955-4653"
---

# Faculty Appointments

```{=latex}
\cvsubhead{Current Appointments:}
\begin{cvroles}
\cvrole{Associate Professor of Anesthesiology}{Medical College of Wisconsin}{2024 -- present}
\cvrole{Secondary appointment in Neurology}{Medical College of Wisconsin}{2019 -- present}
\cvrole{Secondary in Biophysics \& Biomed. Eng.}{Medical College of Wisconsin}{2022 -- present}
\cvrole{Co-Director, Anesthesiology Clinomics}{Medical College of Wisconsin}{2024 -- present}
\end{cvroles}

\cvsubhead{Prior Appointments:}
\begin{cvroles}
\cvrole{Director, Med. Student Anesth. Clerkship}{Medical College of Wisconsin}{2021 -- 2025}
\cvrole{Assistant Professor of Anesthesiology}{Medical College of Wisconsin}{2019 -- 2024}
\cvrole{Instructor, Anesthesiology}{Medical College of Wisconsin}{2018 -- 2019}
\end{cvroles}
```

# Education

```{=latex}
\begin{cvroles}
\cvrole{Neuroanesthesiology Fellow}{Medical College of Wisconsin}{7/2018 -- 6/2019}
\cvrole{Resident, Anesthesiology}{Medical College of Wisconsin}{7/2014 -- 6/2018}
\cvrole{Post-doctoral Research Fellow}{Medical College of Wisconsin}{7/2013 -- 7/2014}
\cvrole{Resident, Neurosurgery}{The Ohio State University}{7/2012 -- 6/2013}
\cvrole{M.D.}{Medical College of Wisconsin}{8/2005 -- 5/2012}
\cvroleblock{Ph.D., Biophysics}{Medical College of Wisconsin}{7/2007 -- 6/2010}{\cvnote{Emphasis on Neuroscience and MRI, mentored by Dr. Jeffrey Binder, MD}\cvnote{Dissertation: ``Using fMRI to Improve Outcome in the Surgical Treatment of Temporal Lobe Epilepsy.''}}
\cvrole{B.A., Psychology}{University of Wisconsin, Milwaukee}{8/2001 -- 5/2004}
\end{cvroles}
```

# Additional Training & Certifications

```{=latex}
\cvsubhead{Additional Training:}
\begin{cvroles}
\cvrole{Clinical Research Scholars Program}{CTSI of Southeastern Wisconsin}{2020 -- 2022}
\cvrole{Physician Scientist Training Program}{Dept. of Anesthesiology, MCW}{2014 -- 2018}
\end{cvroles}

\cvsubhead{Certifications:}
\begin{cvroles}
\cvrole{Board certified}{American Board of Anesthesiology}{3/5/20 -- present}
\cvrole{Basic PTE, Testamer}{National Board of Echocardiography}{12/2/2020}
\end{cvroles}
```

# Grants Awarded

```{=latex}
\begin{cvitems}
\cvgrant{Brain-to-text computer interfaces as a precursor for artificial speech restoration in aphasia}
{Co-I (PI: Anderson)}{2024 -- 2026}{AHW Momentum 5520801 | \$250,000 total funds}

\cvgrant{Decoding mental concept identities using electrocorticography}
{PI}{2023 -- 2026}{NIDCD R21 DC021013 | \$585,000 total funds}

\cvgrant{Center for Aphasia Research and Recovery}
{Co-I (PI: Binder)}{2022 -- 2024}{AHW PPG Pilot AHW004327 | \$250,000 direct funds}

\cvgrant{Defining the Neural Dynamics of Concept Retrieval Using Electrocorticography}
{PI}{2020 -- 2022}{Supported by NIH NCATS KL2 TR001438 | \$279,400 direct funds}

\cvgrant{Localizing critical areas for concept identification with fMRI}
{PI}{2021 -- 2022}{AHW CNRP Seed Grant Award | \$20,000 direct funds}
\end{cvitems}
```

# Publications

```{=latex}
\begin{pubs}
```
PMID:41387573

PMID:40850667

PMID:40424645

PMID:36473300

PMID:36379420

PMID:35933744

PMID:35410903

PMID:33563374

PMID:32780878

PMID:32593799

PMID:32179500

PMID:31314747

PMID:29737490

PMID:29211656

PMID:28540741

PMID:28179469

PMID:27993673

PMID:26966139

PMID:25863238

PMID:25750259

PMID:24021837

PMID:22171207

PMID:20884358

```{=latex}
\cvpub{Gross WL (2010). Using fMRI to Improve Outcome in the Surgical Treatment of Temporal Lobe Epilepsy. PhD dissertation, Medical College of Wisconsin.}
```

PMID:20167415

PMID:18033621

PMID:17690338

PMID:16839289

```{=latex}
\end{pubs}
```

# Invited Lectures

```{=latex}
\begin{cvitems}
\cvlecture{Toward an Implanted Language Neuroprosthesis for Severe Aphasia}
{Visiting professor, University of Madison Dept. of Anesthesiology Grand Rounds}{2/7/2024}

\cvlecture{Beyond Wernicke’s – Advancing our Approach to Preventing Language Deficits}
{Invited lecture, University of Washington, Seattle}{6/8/2021}
\end{cvitems}
```

# Teaching & Mentoring
```{=latex}
\cvsubhead{PhD Mentees \& Committees}
\begin{cvroles}
\cvrole{Dewan Imran Ahmed (Mentee)}{MCW, Biomed. Eng.}{2026 -- present}
\cvrole{Alex Skitowski (Mentee)}{MCW, Biophysics}{2024 -- present}
\cvrole{Aleksandra Winiarz (Committee)}{MCW, Biophysics}{2021 -- present}
\cvroleblock{Stephen Mazurchuk (Committee)}{MCW, Biophysics}{Graduated 2024}{\cvnote{Dissertation: ``An Experiential Account of Semantic Category Organization in the Brain''}}
\cvroleblock{Alexander Helfand (Committee)}{MCW, Biophysics}{Graduated 2020}{\cvnote{Dissertation: ``Toward a gold standard: An outcomes-based assessment of fMRI-based methods for probing language laterality in the brain''}}
\end{cvroles}

\cvsubhead{Postdoctoral Fellows}
\begin{cvroles}
\cvrole{Sunil Mathew, PhD}{MCW, Anesthesiology}{2025 -- present}
\end{cvroles}

\cvsubhead{Student Mentees (Residents \& Graduate)}
\begin{cvroles}
\cvrole{Michelle Kassel (PhD)}{UWM}{2018 -- 2019}
\cvrole{Kevin Beitz (Resident)}{MCW}{2019 -- 2023}
\cvrole{Pooria Bakheshi (Resident)}{MCW}{2020 -- 2021}
\cvrole{Saagar Sheth (Resident)}{MCW}{2021 -- 2025}
\cvrole{Marcus Cooley (Resident)}{MCW}{2022 -- 2026}
\end{cvroles}

\cvsubhead{Medical Students}
\begin{cvroles}
\cvrole{Ying Qiu Zhou}{MCW}{2013 -- 2014}
\cvrole{Ipsit Shah}{MCW}{2021 -- 2024}
\cvrole{Ashley Perry}{MCW}{2021 -- 2025}
\cvrole{Benjamin Maruska}{MCW}{2021 -- 2024}
\end{cvroles}

\cvsubhead{Curriculum Development}
\begin{cvroles}
\cvrole{Fusion Curriculum}{MCW (Anesthesiology)}{2021 -- 2024}
\cvroleblock{Adjunct Faculty}{Carroll University}{9/2013 -- 5/2014}{\cvnote{PTH 560 - Pathology, PTH 405 - Neuroscience}}
\end{cvroles}
```

# Awards
```{=latex}
\begin{cvitems}
\cvline{Fellow Teacher of the Year MCW Anesthesia Residents 2018--2019}
\cvline{Outstanding Medical Student Teacher MCW 2018, 2020}
\cvline{Supportive Faculty Award MCW Dept. of Anesthesiology 2020--2021}
\cvline{Outstanding Educational Leader Award MCW 2025}
\end{cvitems}
```

# Professional Societies
```{=latex}
\begin{cvpairs}
\cvpair{Society for Neuroscience in Anesthesia and Critical Care}{since 2018}
\cvpair{American Society of Anesthesiologists}{since 2014}
\cvpair{Congress of Neurological Surgeons}{2012 -- 2014}
\cvpair{American Association of Neurological Surgeons}{2009 -- 2014}
\cvpair{Society for Neuroscience}{since 2003}
\end{cvpairs}
```

# Journal Reviews / Study Sections
```{=latex}
\begin{cvpairs}
\cvpair{NIH, Special Emphasis Panel: Language, Cognition, and Motor; ZRG1 BP-G(91)}{1/2026}
\cvpair{NIH, Neurotechnology and Vision Study Section; NV-C(15)}{6/2025}
\cvpair{NIH, Language and Communications Study Section; LCOM (Early Career Rev.)}{2/2024}
\cvpair{Journal of Neurosurgical Anesthesia (Editorial Board Member)}{7/2023 -- 7/2025}
\cvpair{Journal of Neurosurgical Anesthesia (Reviewer)}{since 5/2020}
\cvpair{NeuroImage (Reviewer)}{since 3/2020}
\end{cvpairs}
```
